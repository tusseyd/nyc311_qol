...
console_output_file <- file.path(console_dir, "old_jds_quality_console.txt")
...
# Step 1: Copy only incident_zip and year_month fields from cleaned_data
zip_check_data <- cleaned_data[, .(incident_zip, year_month)]
...
# Step 3: Identify malformed ZIPs (not 5 digits, excluding NAs)
invalid_zip_table <- zip_check_data[
  !is.na(incident_zip) & !grepl("^\\d{5}$", incident_zip_5),
  .N,
...
cat("\n", strrep("=", 80), "\n")
cat("MALFORMED ZIP CODE ANALYSIS\n")
cat(strrep("=", 80), "\n\n")

cat(sprintf("Total malformed ZIP codes: %s\n", 
            format(sum(invalid_zip_table$N), big.mark = ",")))
cat(sprintf("Unique malformed values: %s\n\n", 
            format(nrow(invalid_zip_table), big.mark = ",")))

cat("TOP 20 MOST COMMON MALFORMED ZIP CODES:\n")
cat(strrep("-", 80), "\n")
print(invalid_zip_table[1:min(20, nrow(invalid_zip_table))], nrows = Inf)

cat("\n")
if (nrow(invalid_zip_table) > 20) {
  cat(sprintf("... and %s more unique malformed values\n", 
              format(nrow(invalid_zip_table) - 20, big.mark = ",")))
}
cat("\n")
...
incident_invalid_records <- incident_zip_results[[3]]

# Create Pareto table
pareto_table <- incident_invalid_records[, .N, by = incident_zip][order(-N)]
pareto_table[, cumulative_count := cumsum(N)]
pareto_table[, cumulative_pct := 100 * cumulative_count / sum(N)]
pareto_table[, pct := 100 * N / sum(N)]

# Print Pareto analysis
cat("\n", strrep("=", 80), "\n")
cat("PARETO ANALYSIS: INVALID ZIP CODES\n")
cat(strrep("=", 80), "\n\n")

cat(sprintf("Total invalid records: %s\n", 
            format(sum(pareto_table$N), big.mark = ",")))
cat(sprintf("Unique invalid values: %s\n\n", 
            format(nrow(pareto_table), big.mark = ",")))

# Print full table
cat("ALL INVALID ZIP CODES (Full Pareto Table):\n")
cat(strrep("-", 80), "\n")
cat(sprintf("%-20s %10s %8s %12s %12s\n", 
            "Invalid ZIP", "Count", "Pct %", "Cum Count", "Cum Pct %"))
cat(strrep("-", 80), "\n")

# Print all entries
for (i in 1:nrow(pareto_table)) {
  cat(sprintf("%-20s %10s %7.2f%% %12s %10.2f%%\n",
              pareto_table$incident_zip[i],
              format(pareto_table$N[i], big.mark = ","),
              pareto_table$pct[i],
              format(pareto_table$cumulative_count[i], big.mark = ","),
              pareto_table$cumulative_pct[i]))
}
...
# Step 1: Copy only incident_zip and year_month fields from cleaned_data
community_board_data <- cleaned_data[, .(community_board, year_month)]
...
community_board_invalid <- community_board_results[[3]]

fwrite(community_board_invalid, 
       file = file.path(analytics_dir, "community_board_invalid_results.csv"))
