analyze_skewed_durations <- function(
    DT,
    duration_col        = "duration_sec",         # Changed to seconds
    chart_dir           = NULL,
    minimum_cutoff_sec  = 2,                      # 2 seconds
    upper_cutoff_sec    = 8 * 86400,              # 8 days in seconds
    print_summary       = TRUE,
    create_plots        = TRUE,
  ...
  trunc_mad     <- stats::mad(truncated_data$val, constant = 1.4826)
  ...
  # Log-normal (lower)
  log_vals      <- log(truncated_data$val)
  ...
  # Thresholds should not be negative
  ...
  if (print_summary) {
    ...
    reasonable_methods <- methods_dt[threshold_seconds > 0 & threshold_seconds < 3600]  # < 1 hour
  ...
  if (create_plots) {
    ...
    # Plot 2: Thresholds by method (show "reasonable" cutoffs < 1 hour)
    reasonable_for_plot <- methods_dt[threshold_seconds > 0 & threshold_seconds < 3600]
