plot_duration_histogram <- function(
  ...
  if (is.null(title)) {
    title <- sprintf(
      "Distribution of Durations (%s to %s %s)",
  ...
  if (create_cumulative) {
    ...
    cum_title <- if (is.null(title)) {
      paste("Cumulative Distribution of Durations", paste0("(", data_units, ")"))
    } else {
      paste("Cumulative", title)
    }
    ...
    p2 <- ggplot2::ggplot(hist_data_cum, ggplot2::aes(x = duration_binned, y = cum_pct)) +
      ...
      ggplot2::geom_hline(
        yintercept = c(90), color = c("#CC79A7"),
    ...
    ggplot2::ggsave(outfile2,
      plot = p2, width = width_in, height = height_in,
      dpi = 300, device = grDevices::cairo_pdf
    )
  }
  ...
  m_sec <- mean(vals_sec)
  md_sec <- stats::median(vals_sec)
  s_sec <- stats::sd(vals_sec)
  cat(sprintf(
  ...
  cat(sprintf(
    "Bin width: %g %s (%s seconds, %.6f days)\n",
