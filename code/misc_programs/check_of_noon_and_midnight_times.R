...
agency_col_orig   <- "Agency"  # <--- add this
valid_date_columns <- c(
  "Created Date", "Closed Date", "Due Date", "Resolution Action Updated Date"
)

# --- 2. Read only those columns ---
cols_to_read <- c(unique_key_col, agency_col_orig, valid_date_columns)  # <--- include Agency

raw_data_dir <- file.path(data_dir, "raw_data")


temp_raw_data <- fread(
  file.path(raw_data_dir, "smaller_test_data.csv"),
  select = cols_to_read,
...
temp_raw_data <- modify_column_names(temp_raw_data)
# after standardization:
#   "Unique Key" -> "unique_key"
#   "Agency"     -> "agency"
#   date fields  -> lower_snake
...
# --- 4. Containers for diagnostics ---
parse_failures <- list()
time_summary   <- list()
fmt       <- "%m/%d/%Y %I:%M:%S %p"
local_tz  <- "America/New_York"

# --- 5. Loop through each date field (collect failures only) ---
for (col in valid_date_columns) {
  ...
  if (length(bad_idx)) {
    ...
    parse_failures[[col]] <- data.table(
      ...
      agency         = if ("agency" %in% names(temp_raw_data)) temp_raw_data$agency[bad_idx] else NA_character_,
...
# --- 6. Combine and review all failures ---
if (length(parse_failures)) {
  ...
  # --- 7. Run DST fix across all failures in one pass ---
  cat("\n--- Running DST Spring-Forward Fix for All Failures ---\n")
  
  # We’ll loop again per affected column (but now just for the bad records)
  for (col in names(parse_failures)) {
    ...
    dst_result <- fix_dst_spring_forward(
      ...
      fmt           = fmt,
      local_tz      = local_tz,
      agency        = if ("agency" %in% names(temp_raw_data)) temp_raw_data$agency else NULL,
...
# --- 8. Summarize true midnight / noon times (after DST fixes) ---
for (col in valid_date_columns) {
  new_col <- paste0(gsub(" ", "_", tolower(col)), "_posix")
...
# --- 10. Replace original columns with their _posix versions and delete extras ---
...
for (col in valid_date_columns) {
  ...
  if (posix_col %in% names(temp_raw_data)) {
    ...
    temp_raw_data[, (posix_col) := NULL]   # remove the _posix column
  } else {
